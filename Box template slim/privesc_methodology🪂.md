...
- [ ] Check open ports not available from the outside and if the target is communicating with someone else
	...
	```
### Credential Access
- [x] Try known passwords
- [x] Search creds from config files (Try different word other the PASSWORD, e.g: pass, passwd, pwd, user, usr, username, secret, cred, credential, auth):
	```bash
	grep \--color\=auto \-rnw '/' \-ie "PASSWORD" \--color\=always 2> /dev/null
	find . \-type f \-exec grep \-i \-I "PASSWORD" {} /dev/null
	locate password | more
	```
- [x] Search creds from local DBs
- [x] Search creds from bash history:
	```bash
	history
	cat ~/.bash_history
	```
